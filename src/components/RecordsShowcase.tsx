...
export const RecordsShowcase: React.FC = () => {
  ...
  return (
    <section id="records" className="max-w-7xl mx-auto px-6 lg:px-12 py-16 border-t border-white/5 scroll-mt-20">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-12 gap-6">
        ...
        {/* Format Filter */}
        <div className="flex items-center gap-1 bg-[#07090d] p-1 border border-white/5">
          {(['ALL', 'Test', 'ODI', 'T20I'] as const).map(fmt => (
            <button
              key={fmt}
              onClick={() => setSelectedFormat(fmt)}
              className={`px-3 py-1.5 text-[10px] font-bold uppercase transition-all tracking-wider ${
                selectedFormat === fmt
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {filteredCenturies.map((c, i) => (
          <div key={i} className="bg-[#07090d] border border-white/5 p-6 space-y-4 relative group hover:border-[#d3122a]/50 transition-all">
            <div className="flex items-center justify-between">
              <span className="text-[9px] font-mono font-bold uppercase px-2.5 py-1 bg-[#d3122a]/10 text-[#d3122a] border border-[#d3122a]/20">
                {c.format}
              </span>
              <span className="text-[10px] text-white/30 font-mono">{c.year}</span>
            </div>

            <div>
              <p className="text-[10px] text-white/40 uppercase font-mono tracking-widest">vs {c.opponent}</p>
              <p className="text-3xl font-black text-[#d3122a] mt-1 font-brand">{c.score}</p>
            </div>
      ...
      <div className="mt-12 grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-[#07090d] border border-white/5 p-6 border-l-4 border-l-[#d3122a]">
          <h4 className="text-[10px] uppercase font-mono text-[#d3122a] font-bold mb-1">Fastest to 10,000 ODI Runs</h4>
          <p className="text-2xl font-black text-white font-brand">205 Innings</p>
          <p className="text-xs text-white/50 mt-2 font-light">Broke Sachin Tendulkar's record (259 innings) by 54 innings.</p>
        </div>

        <div className="bg-[#07090d] border border-white/5 p-6 border-l-4 border-l-[#d3122a]">
          <h4 className="text-[10px] uppercase font-mono text-[#d3122a] font-bold mb-1">Most Double Centuries by Indian in Tests</h4>
          <p className="text-2xl font-black text-white font-brand">7 Double 100s</p>
          <p className="text-xs text-white/50 mt-2 font-light">Surpassed Virender Sehwag and Sachin Tendulkar (6 each).</p>
        </div>

        <div className="bg-[#07090d] border border-white/5 p-6 border-l-4 border-l-[#d3122a]">
          <h4 className="text-[10px] uppercase font-mono text-[#d3122a] font-bold mb-1">Most Runs in Single IPL Season (RCB)</h4>
          <p className="text-2xl font-black text-white font-brand">973 Runs (2016)</p>
          <p className="text-xs text-white/50 mt-2 font-light">Included 4 centuries in a single tournament season for RCB.</p>
        </div>
      </div>
    </section>
  );
